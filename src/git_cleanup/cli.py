...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "column (e.g. --filter brent); boolean flags: mine, merged, local, remote, "
            "gone, worktree (prefix ! to negate, or =true/=false); age>N/age<N (days, "
            "or 6m/1y); column=value substring match (branch, author, issue, status; "
            "!= excludes); '|' ORs alternatives inside one text term (author=sam|chris, "
            "or bare sam|chris); an empty value tests whether the column is set at all "
            "('status=' for no status, 'status!=' for any). Quote specs containing >, !, "
            "or | (e.g. --filter 'mine,merged=true,author=sam|chris'); "
